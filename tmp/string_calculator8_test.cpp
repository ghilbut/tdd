...
TEST(StringCalculatorTest, when_negative_number_is_used_then_runtime_exception_is_thrown) {
    EXPECT_THROW(StringCalculator::Add("3,-6,15,18"), StringCalculator::Exception*);
}

TEST(StringCalculatorTest, when_negative_numbers_are_used_then_runtime_exception_is_thrown) {
    try {
        StringCalculator::Add("3,-6,15,-18");
        FAIL();
    } catch (StringCalculator::Exception* e) {
        EXPECT_STREQ("negative not allowed - [-6,-18]", e->what());
    } catch (...) {
        FAIL();
    }
}
